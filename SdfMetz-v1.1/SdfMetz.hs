...
import Data.Relation.GraphViz (printGraphWith, printRel)
...
data SyntaxFormalism = SdfGrammar | DmsGrammar
...
options =
   ...
   , Option "f" ["syntax-formalism"]
        (ReqArg
           ...
           "Formalism")
           "Grammar formalism used as input (can be one of {\"sdf\", \"dms\"})"
   , Option "m" ["metrics"]
        (NoArg (\opt -> do 
...
dmsTable = "Language/DmsGrammar/dms-syntax.def.tbl"
