...
import Data.Map (empty, insertWith, fold)
...
data SizeMetrics = SizeMetrics
   ...
   , mcc  :: Float
     -- | McCabe cyclomatic complexity per non-terminal (average). Measures the
     --   number of linearly independent paths through the flow graph of the 
     --   defined non-terminal
   , mccN :: Float
     -- | McCabe cyclomatic complexity per production rule (average). Measures
     --   the number of linearly independent paths through the flow graph of
     --   the production rule.
   , mccP :: Float
...
sizeMetrics sm = 
   let termM = Metric "Number of unique terminals" "TERM" (show $ term sm)
       ...
       mccnM = Metric "Average McCabe per non-terminal" "MCCN" (show $ mccN sm)
       mccpM = Metric "Average McCabe per production" "MCCP" (show $ mccP sm)
       avsnM = Metric "Average RHS per non-terminal" "AVSN" (show $ avsN sm)
       avspM = Metric "Average RHS per production" "AVSP" (show $ avsP sm)
       metrics = [termM, varM, uvarM, prodM, mccM, mccnM, mccpM, avsnM, avspM]
...
calcTERM :: SDF -> Float
calcTERM sdf = fromIntegral $ (size literals) + (size usedLexSorts)
   where literals     = fromList $ collectTerminals rhsSymbs
         defLexSorts  = fromList $ concatMap (getSortName . getSort) $
                        collectLexProductions sdf
         usedSorts    = fromList $ collectSortNames rhsSymbs
         usedLexSorts = defLexSorts `intersection` usedSorts
...
calcVAR = fromIntegral . size . fromList . concatMap (getSortName . getSort)
...
calcUVar = fromIntegral . size . fromList . 
...
calcMCC2 = 
   let -- calculates a bag for the defined sorts. This will allow to know
       ...
       bagAritySorts = foldr (\e fm -> insertWith (+) e 1 fm) empty
